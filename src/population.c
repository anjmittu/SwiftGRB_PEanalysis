...
void GeneratePopulationTwoBreak(double population[], long int population_size, double n0, double n1, double n2, double n3, double z1,
				double z2, double x, double y, double log_lum_star, double zpop[], long int *seed)
{
	/*
		Input parameters:
		0  log_L 					log-luminosity
	...
	for ( ipop=0; ipop<population_size; ipop++ )
	{
		...
		do {
			if (runargs.flatzpop) {
				...
				//z = redshift_distribution(seed);
			  if (runargs.twobreak) {
			    z = Redshift_rejection_samplerTwoBreak(seed, n0, n1, n2, n3, z1, z2);
			  } else {
			    z = Redshift_rejection_sampler(seed, n0, n1, n2, z1);
			  }
			}
